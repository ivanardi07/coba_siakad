...
<div class="form-group row align-items-center" :class="{'has-danger': errors.has('NIS'), 'has-success': fields.NIS && fields.NIS.valid }">
    <label for="NIS" class="col-form-label text-md-right" :class="isFormLocalized ? 'col-md-4' : 'col-md-2'">{{ trans('admin.user.columns.NIS') }}</label>
        <div :class="isFormLocalized ? 'col-md-4' : 'col-md-9 col-xl-8'">
        <input type="text" v-model="form.NIS" v-validate="''" @input="validate($event)" class="form-control" :class="{'form-control-danger': errors.has('NIS'), 'form-control-success': fields.NIS && fields.NIS.valid}" id="NIS" name="NIS" placeholder="{{ trans('admin.user.columns.NIS') }}">
